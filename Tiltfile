BAZEL_RUN_CMD = "bazel run --platforms=@io_bazel_rules_go//go/toolchain:linux_amd64 --sandbox_debug %s %s"
...
k8s_yaml(bazel_k8s("//greater-server:k8s"))

bazel_build('bazel/greater-server', "//greater-server:image", "-- --norun")

k8s_resource('varowner-greater', port_forwards=50051)
